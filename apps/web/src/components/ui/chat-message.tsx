    ...
    <div className={cn("flex flex-col", isUser ? "items-end" : "items-start")}>
      ...
      <div className={cn(chatBubbleVariants({ isUser, animation }), className)}>
        ...
        {role === "assistant" && actions ? (
          <div className="absolute flex p-1 space-x-1 transition-opacity border rounded-lg opacity-0 -bottom-4 right-2 bg-background text-foreground group-hover/message:opacity-100">
            {actions}
          </div>
        ) : null}
      </div>
        ...
        switch (invocation.state) {
          ...
          case "call":
            return (
              <div
                key={index}
                className="flex items-center gap-2 px-3 py-2 text-sm border rounded-lg bg-muted text-muted-foreground"
              >
